...
parser.add_argument(
    "--vrf-skey",
    dest="skey",
    help="provide the path to the pool.vrf.skey file or the raw skey (128 hex characters)",
    required=True,
)
parser.add_argument(
    "--sigma",
    dest="sigma",
    type=float,
    help="the controlled stake sigma value of the pool [e.g. 0.0034052348379780869]",
)
parser.add_argument(
    "--pool-id", dest="poolId", help="the Pool ID to fetch from the Epoch API"
)
parser.add_argument(
    "--epoch", dest="epoch", type=int, help="the epoch number [e.g. 221]"
)
parser.add_argument(
    "--epoch-nonce", dest="eta0", help="the epoch nonce to check"
)
parser.add_argument(
    "--d-param",
    dest="d",
    type=float,
    help="the current decentralization parameter [e.g. 0.0 - 1.0]",
)
parser.add_argument(
    "-bft",
    action="store_true",
    help="if specified will also calculate slots stolen by BFT due to d not being 0",
)
parser.add_argument(
    "--tz",
    dest="tz",
    default="America/Los_Angeles",
    help="the local timezone name [Default: America/Los_Angeles]",
)
...
if poolId is not None:
    ...
    if epoch is not None:
        ...
        print(
            "\033[94m[INFO]:\033[0m No epoch provided, using latest known epoch."
        )
    ...
    try:
        ...
        print(
            "\033[1;31m[WARN]:\033[0m Unable to fetch data from the sigma API."
        )
        exit()
    try:
        ...
        print(
            "\033[1;31m[ERROR]:\033[0m One or more data points from the API are missing or invalid. Please try again."
        )
        parse.format_help()
        parser.print_help()
        exit()
    ...
    try:
        ...
        print(
            "\033[1;31m[WARN]:\033[0m Unable to fetch data from the epoch API."
        )
    ...
    try:
        ...
        print(
            "\033[1;31m[ERROR]:\033[0m One or more arguments are missing or invalid. Please try again."
        )
        parser.format_help()
        parser.print_help()
        exit()
    print(
        "Checking leadership log for Epoch",
        epoch,
        "[ d Param:",
...
if eta0 == "TBD":
    print(
        "\033[1;31m[ERROR]:\033[0m You're a bit early. The epoch nonce for Epoch",
        epoch,
        "isn't ready yet! Try again later.",
    )
    exit()
...
def vrfEvalCertified(seed, tpraosCanBeLeaderSignKeyVRF):
    if isinstance(seed, bytes) and isinstance(
        tpraosCanBeLeaderSignKeyVRF, bytes
    ):
        proof = create_string_buffer(
            libsodium.crypto_vrf_ietfdraft03_proofbytes()
        )

        libsodium.crypto_vrf_prove(
            proof, tpraosCanBeLeaderSignKeyVRF, seed, len(seed)
        )
    ...
    if slotLeader:
        ...
        if overlaySlot:
            ...
            print(
                ...
                + ", Cumulative stolen blocks due to d param: "
                + str(stolencount)
            )
        else:
            slotcount += 1
