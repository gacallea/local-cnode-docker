...
if [ ! -f "${metafile}" ]; then
  echo -e "The specified MetaDataFileName.json (${metafile} file does not exist. Please try again."
  exit 1
fi
...
utx0=$(${cardanocli} query utxo --address ${sendFromAddr} --cardano-mode ${magicparam})
...
${cardanocli} query protocol-parameters --cardano-mode ${magicparam} > protocol-parameters.json
...
#Generate Dummy-TxBody file for fee calculation
	...
	if [[ ${rxcnt} == 1 ]]; then  #Sending ALL funds  (rxcnt=1)
                        ${cardanocli} transaction build-raw ${txInString} --tx-out ${dummyShelleyAddr}+0 --ttl ${ttl} --fee 0 --metadata-json-file ${metafile} --out-file ${txBodyFile}
			checkError "$?"
                        else  #Sending chosen amount (rxcnt=2)
                        ${cardanocli} transaction build-raw ${txInString} --tx-out ${dummyShelleyAddr}+0 --tx-out ${dummyShelleyAddr}+0 --metadata-json-file ${metafile} --ttl ${ttl} --fee 0 --out-file ${txBodyFile}
			checkError "$?"
	fi
fee=$(${cardanocli} transaction calculate-min-fee --tx-body-file ${txBodyFile} --protocol-params-file protocol-parameters.json --tx-in-count ${txcnt} --tx-out-count ${rxcnt} ${magicparam} --witness-count 1 --byron-witness-count 0 | awk '{ print $1 }')
checkError "$?"
...
rm ${txBodyFile} 2> /dev/null
if [[ ${rxcnt} == 1 ]]; then  #Sending ALL funds  (rxcnt=1)
			${cardanocli} transaction build-raw ${txInString} --tx-out ${sendToAddr}+${lovelacesToSend} --ttl ${ttl} --fee ${fee} --metadata-json-file ${metafile} --out-file ${txBodyFile}
			checkError "$?"
			else  #Sending chosen amount (rxcnt=2)
			${cardanocli} transaction build-raw ${txInString} --tx-out ${sendToAddr}+${lovelacesToSend} --tx-out ${sendFromAddr}+${lovelacesToReturn} --metadata-json-file ${metafile} --ttl ${ttl} --fee ${fee} --out-file ${txBodyFile}
			checkError "$?"
fi
...
if ask "\e[33mDoes this look good for you, continue ?" N; then
	...
	${cardanocli} transaction submit --tx-file ${txFile} --cardano-mode ${magicparam}
	checkError "$?"
	echo -e "\e[32mDONE\n"
fi
