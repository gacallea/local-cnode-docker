...
parser = argparse.ArgumentParser(
    description="Calculate the sigma value of the specified pool. If you do not provide the path to a ledger file via the --ledger option then the script will look for a ledger.json file in the current directory"
)
...
parser.add_argument(
    "--next",
    action="store_true",
    help="if specified will provide sigma for the next epoch instead of the current epoch",
)
...
if not path.exists(ledger):
    ...
    print(
        "Use: \033[1;34mcardano-cli shelley query ledger-state --mainnet --out-file ledger.json\033[0m to export one!"
    )
    exit()
